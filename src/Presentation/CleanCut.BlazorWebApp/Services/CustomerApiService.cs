...
public class CustomerApiService : ICustomerApiService
{
    ...
    public CustomerApiService(HttpClient httpClient, ILogger<CustomerApiService> logger, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async Task<CustomerInfo?> GetCustomerByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var url = $"{BaseUrl}/api/customers/{id}";
            _logger.LogInformation("Making GET request to: {Url}", url);
            var response = await _httpClient.GetAsync(url, cancellationToken);
